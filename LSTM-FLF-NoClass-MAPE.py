...
dictionary  = evaluate(xtest,"Open","sMAPE_result.txt")
...
def plot_validation_loss(filename="validation_loss.pdf"):
    ...
    for label, pair in history_dictionary.items():
        ...
        plt.title('Candle validation loss(sMAPE)')
        plt.ylabel('loss')
        plt.xlabel('epochs')
        plt.legend(loc='best')
...
#%%
plot_validation_loss("sMAPE_validation_loss.pdf")

def save_pred(path= ""):
        for k, v in predictions.items():
            file_name = path + "./Org_FLF_sMAPE" + k + ".csv"
            df = pd.DataFrame()
            df['Open'] = v[:,0]
            df['High'] = v[:,1]
            df['Low'] =  v[:,2]
...
plot_error(dictionary,3,'sMAPE_my_error_plot.pdf')
...
plot("Open","Open_sMAPE.pdf")
plot("High","High_sMAPE.pdf")
plot("Low","Low_sMAPE.pdf")
plot("Close","Close_sMAPE.pdf")