...
dictionary  = evaluate(xtest,"Open","MSE_result.txt")
...
#%%
plot_validation_loss("MSE_validation_loss.pdf")

def save_pred(path= ""):
        for k, v in predictions.items():
            file_name = path + "./Org_FLF_MSE" + k + ".csv"
            df = pd.DataFrame()
            df['Open'] = v[:,0]
            df['High'] = v[:,1]
            df['Low'] =  v[:,2]
...
plot_error(dictionary,3,'MSE_my_error_plot.pdf')
...
plot("Open","Open_MSE.pdf")
plot("High","High_MSE.pdf")
plot("Low","Low_MSE.pdf")
plot("Close","Close_MSE.pdf")