...
data = pd.read_csv('/Users/grayman/Documents/PhD/Data/2023.csv',sep=',',index_col='Date',parse_dates=True)
...
dictionary  = evaluate(xtest,"Open","MAE_result.txt")
...
#%%
plot_validation_loss("MAE_validation_loss.pdf")

def save_pred(path= ""):
        for k, v in predictions.items():
            file_name = path + "./Org_FLF_MAE" + k + ".csv"
            df = pd.DataFrame()
            df['Open'] = v[:,0]
            df['High'] = v[:,1]
            df['Low'] =  v[:,2]
...
plot_error(dictionary,3,'MAE_my_error_plot.pdf')
...
plot("Open","Open_MAE.pdf")
plot("High","High_MAE.pdf")
plot("Low","Low_MAE.pdf")
plot("Close","Close_MAE.pdf")